...
from .online_status_store import mark_device_online
...
def _apply_coordinate_direction(value: float | None, direction: str | None) -> float | None:
    if value is None:
        return None
    ...
    if normalized_direction in ('S', 'W'):
        return -abs(value)
    if normalized_direction in ('N', 'E'):
        return abs(value)
    return value


def _select_preferred_location(
    boat_payload: dict | None,
    rtk_payload: dict | None,
) -> Tuple[float | None, float | None]:
    if rtk_payload:
        rtk_latitude = rtk_payload.get('latitude')
        rtk_longitude = rtk_payload.get('longitude')
        if rtk_latitude is not None and rtk_longitude is not None:
            return rtk_latitude, rtk_longitude

    if not boat_payload:
        return None, None

    return boat_payload.get('latitude'), boat_payload.get('longitude')
...
def parse_rtk_payload(input_text: str) -> dict:
    ...
    return {
        ...
        'rtk_latitude': rtk_latitude,
        'rtk_latitude_direction': rtk_latitude_direction,
        'rtk_longitude': rtk_longitude,
        'rtk_longitude_direction': rtk_longitude_direction,
        'latitude': _apply_coordinate_direction(rtk_latitude, rtk_latitude_direction),
        'longitude': _apply_coordinate_direction(rtk_longitude, rtk_longitude_direction),
...
class ShipGatewayService:
    ...
    def _expected_ports(self) -> List[int]:
        return list(range(self.port_start, self.port_start + self.ship_count * 2))

    def _detect_bound_ports(self) -> List[int]:
        bound_ports: List[int] = []

        for port in self._expected_ports():
    ...
    def status(self) -> dict:
        ...
        listening_ports = self._detect_bound_ports()
        is_running = self._running or len(listening_ports) == len(self._expected_ports())
        last_depth_packets = {
    ...
    def _mapped_ship_model(self, ship_port: int) -> str:
        canonical_port = self._canonical_ship_port(ship_port)
        return self.ship_port_model_map.get(str(canonical_port)) or str(canonical_port)

    def _resolve_ship_model(self, ship_port: int) -> str:
        # 状态场景固定按端口映射，不再依赖报文主动探测船型。
        return self._mapped_ship_model(ship_port)

    def _save_boat_track_record(
        self,
        ship_model: str,
        ship_port: int,
        boat_payload: dict,
        rtk_payload: dict | None = None,
    ) -> None:
        latitude, longitude = _select_preferred_location(boat_payload, rtk_payload)
    ...
    def _persist_packets(self, ship_port: int, packets: List[dict]) -> None:
        ...
        for packet in packets:
            ...
            try:
                ...
                mapped_model = self._mapped_ship_model(canonical_ship_port)
            ...
            with self._lock:
                self.reported_ship_model_by_port[canonical_ship_port] = mapped_model
                self.reported_ship_model_by_port[canonical_ship_port + 1] = mapped_model
                self.last_boat_packet_by_port[ship_port] = boat_payload
                cached_rtk_payload = self.last_rtk_packet_by_port.get(ship_port)

            # 跨进程在线状态：写入 Redis 心跳
            mark_device_online(mapped_model)

            try:
                self._save_boat_track_record(mapped_model, canonical_ship_port, boat_payload, cached_rtk_payload)
            except Exception as exc:
            ...
            try:
                if packet_type == 'W':
                    ...
                    with self._lock:
                        ...
                        boat_payload = self.last_boat_packet_by_port.get(ship_port)
                    if boat_payload:
                        rtk_latitude, rtk_longitude = _select_preferred_location(None, rtk_payload)
                        boat_latitude = boat_payload.get('latitude')
                        boat_longitude = boat_payload.get('longitude')
                        if (
                            rtk_latitude is not None
                            and rtk_longitude is not None
                            and (rtk_latitude != boat_latitude or rtk_longitude != boat_longitude)
                        ):
                            self._save_boat_track_record(ship_model, canonical_ship_port, boat_payload, rtk_payload)
            except Exception as exc:

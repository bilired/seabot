...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            user_models = {
                str(model).strip()
                for model in DroneDevice.objects.filter(owner=request.user).values_list('model', flat=True)
                if str(model).strip()
            }
            online_models = {
                pkt.get('ship_model') or f'船体-{port}'
                for port, pkt in last_boat_packets.items()
                if str(port) in online_ports_set and (pkt.get('latitude') is not None) and ((pkt.get('ship_model') or f'船体-{port}') in user_models)
            }
            ...
            today_activities = UserActivity.objects.filter(user=request.user, created_at__gte=today_start).count()

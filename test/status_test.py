...
HOST, PORT = '8.130.132.91', 9003  # 8.130.132.91，127.0.0.1
ENABLE_HEARTBEAT = True
HEARTBEAT_INTERVAL_SECONDS = 5
ENABLE_RTK = True
RTK_INTERVAL_SECONDS = 1
...
battery   = 24.6   # 电池电压（V）
depth     = 1.5    # 水深（m）
temperature = 26.0  # 水温（°C）
rtk_elevation = 8.3  # RTK 高程（m）
WATER_STATES = ['normal', 'sampling', 'lifting', 'recovering', 'abnormal']


def direction_for_latitude(value: float) -> str:
    return 'N' if value >= 0 else 'S'


def direction_for_longitude(value: float) -> str:
...
while True:
    ...
    speed = 0.9 + 0.6 * math.sin(packet_count * 0.1)
    ...
    # RTK 水深、水温、高程做轻微波动，便于观察上报数据变化
    depth = round(1.5 + 0.4 * math.sin(packet_count * 0.07), 2)
    temperature = round(26.0 + 1.2 * math.sin(packet_count * 0.05), 1)
    rtk_elevation = round(8.3 + 0.3 * math.cos(packet_count * 0.06), 2)
    ...
    # 船型直接写死或由外部传入，这里示例用'DL-3022'
    payload_text = (
        f'DL-3022|{ts}|{latitude:.6f}|{longitude:.6f}'
    ...
    if ENABLE_RTK and (time.time() - last_rtk_at >= RTK_INTERVAL_SECONDS):
        rtk_latitude = latitude + 0.00018
        rtk_longitude = longitude - 0.00012
        rtk_payload_text = (
            f'{depth:.2f}|{abs(rtk_latitude):.6f}|{direction_for_latitude(rtk_latitude)}'
            f'|{abs(rtk_longitude):.6f}|{direction_for_longitude(rtk_longitude)}'
            f'|{temperature:.1f}|{rtk_elevation:.2f}'
        )
        rtk_packet = build_packet('0', packet_count, rtk_payload_text.encode('ascii'))
        s.sendall(rtk_packet)
        print(
            f'[RTK ] {ts}  lat={rtk_latitude:.6f} lon={rtk_longitude:.6f}'
            f'  depth={depth:.2f}m  temp={temperature:.1f}C  elev={rtk_elevation:.2f}m'
        )
        packet_count += 1
        last_rtk_at = time.time()

    if ENABLE_HEARTBEAT and (time.time() - last_heartbeat_at >= HEARTBEAT_INTERVAL_SECONDS):
        hb_ts = datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')
        hb_payload_text = (
            f'DL-3022|{hb_ts}|{latitude:.6f}|{longitude:.6f}'
            f'|{course:.0f}|{speed:.1f}|{battery}|heartbeat'
        )
        hb_packet = build_packet('B', packet_count, hb_payload_text.encode('ascii'))
        s.sendall(hb_packet)
        print(f'[HB  ] {hb_ts}  lat={latitude:.6f} lon={longitude:.6f}  water=heartbeat')
        packet_count += 1
